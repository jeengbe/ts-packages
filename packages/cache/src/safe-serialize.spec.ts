import type { SafeJsonOptions } from './safe-serialize.js';
import { safeJsonSerialize } from './safe-serialize.js';
...
describe('safeJsonSerialize', () => {
  ...
  describe("an object with 'undefined' values", () => {
    ...
    allows('with allowUndefined: true', { key: undefined }, { allowUndefined: true });
    disallows('with allowUndefined: false', { key: undefined }, { allowUndefined: false });
  });
...
function allows(description: string, value: unknown, options?: SafeJsonOptions) {
  it(`should serialize ${description}`, () => {
    expect(safeJsonSerialize(value, options)).toMatchSnapshot();
  });
}

function disallows(description: string, value: unknown, options?: SafeJsonOptions) {
