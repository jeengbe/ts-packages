...
> extends EventEmitter<CacheEvents<Entries>> {
   ...
   * ```
   */
  async get<K extends keyof Entries & string>(key: K): Promise<Entries[K] | undefined> {
  ...
  async set<K extends keyof Entries & string>(
    ...
    await this.cacheAdapter.mset([[cacheKey, this.serialize(value, key), ttlMs]]);
  }
  ...
  async mset<I extends readonly Entries[keyof Entries & string][]>(
    items: I,
    makeKey: (this: void, item: I[number], index: number) => keyof Entries & string,
   ...
   */
  async getRemainingTtl(key: keyof Entries & string): Promise<number | undefined> {
    const cacheKey = this.getCacheKey(key);
