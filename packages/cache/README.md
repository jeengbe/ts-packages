...
[![Coverage](https://codecov.io/gh/jeengbe/ts-packages/branch/master/graph/badge.svg?component=cache)](https://app.codecov.io/gh/jeengbe/ts-packages/tree/master/packages/cache)
...
export class CachedMyService implements MyService {
  ...
  async doThing(param: number): Promise<string> {
    return await this.cache.cached(`thing-${param}`, () => this.delegate.doThing(param));
  }
}
...
```ts
declare function expensiveBatchFunction(ids: readonly string[]): Promise<Result[]>;
...
await resultCache.mget(ids.map<`expensive-${string}`>((id) => `expensive-${id}`));
```
